...
export default {
  ...
  setup(props, { emit }) {
    ...
    watch(
      () => props.initialValue,
      () => {
        if (props.behavior === 'radio')
          selectedItems.value = [props.initialValue]
        else selectedItems.value = props.initialValue
      }
    )
    ...
    const selectItem = (itemName) => {
      ...
      if (selectedItemIndex === -1) {
        if (props.behavior === 'checkbox') {
          selectedItems.value.push(itemName)
        } else if (props.behavior === 'radio') selectedItems.value[0] = itemName
...
<template>
  <ul class="product__color--list shortcut--list non__click">
    <li
      v-for="tag in tags"
      :key="tag"
      class="non__click w-[max-content] m-b-.8rem !rtl:m-l-.8rem !ltr:m-r-.8rem !lowercase"
      :class="{
        ...activeClasses,
